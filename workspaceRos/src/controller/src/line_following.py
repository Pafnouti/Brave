...
class Controller():
    ...
    def __init__(self, rosrate=10):
        ...
        self.pub_state = rospy.Publisher('/State', Pose2D, queue_size=10)
    ...
    def _callback_gps(self, msg):
        ...
        self.lym = lat*(float(lat_str[0][0:len(lat_str[0])-2]) + float(
            lat_str[0][len(lat_str[0])-2:])/60. + float(lat_str[1])/10**len(lat_str[1])/60.)
        self.lxm = lon*(float(lon_str[0][0:len(lon_str[0])-2]) + float(
            lon_str[0][len(lon_str[0])-2:])/60. + float(lon_str[1])/10**len(lon_str[1])/60.)
    ...
    def control(self, a, b, m):
        ...
        delta_r = (delta_rmax/pi) * \
            self.sawtooth(thetaBar-theta)  # de -pi/3 a pi/3
    ...
    def main(self):
        ...
        a = array([[self.xa], [self.ya]])
        b = array([[self.xb], [self.yb]])

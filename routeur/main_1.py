...
class Routeur():
    ...
    def run(self, A, B, angle_calcul=180, nb_traj=100, pas=4):

        b = Point(B[0], B[1])
        trajs = [(pointAtDistance(A, self.polaire(2*np.pi*i/nb_traj)*5, 2*np.pi*i/nb_traj)) for i in range(nb_traj)]
        isochrones = [Polygon([el for el in trajs])]
             
     
        while(not isochrones[-1].contains(b) and len(isochrones)<30):
            bounds = list(isochrones[-1].exterior.coords)
            iso = []
            for i in range(len(bounds)):
                angle_next = getAngle(bounds[i-1], bounds[(i+1)%len(bounds)]) - np.pi/2
